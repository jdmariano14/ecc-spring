...
<form name="person_result" action="/persons/result" method="post">
  <fieldset>
    ...
    <c:if test="${not empty minDate}">
      <label for="person_result[min_date]">Min</label>
      <input type="date" name="person[min_date]" value="${minDate}">
      <br>
    </c:if>
    <c:if test="${not empty maxDate}">
      <label for="person_result[max_date]">Min</label>
      <input type="date" name="person[max_date]" value="${maxDate}">
      <br>
    </c:if>
    <c:if test="${not empty minBigDecimal}">
      <label for="person_result[min_big_decimal]">Min</label>
      <input type="number" name="person_result[min_big_decimal]" value="${minBigDecimal}" step=0.01>
      <br>
    </c:if>
    <c:if test="${not empty maxBigDecimal}">
      <label for="person_result[max_big_decimal]">Min</label>
      <input type="number" name="person_result[max_big_decimal]" value="${maxBigDecimal}" step=0.01>
      <br>
    </c:if>
